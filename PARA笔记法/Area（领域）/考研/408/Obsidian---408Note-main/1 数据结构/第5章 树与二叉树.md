...
![树的定义和基本术语.pdf](01.5.1.1+5.1.2树的定义和基本术语.pdf)
...
![二叉树的定义和基本术语.pdf](01.5.2.1_1二叉树的定义和基本术语.pdf)
...
![二叉树的性质.pdf](02.5.2.1_2二叉树的性质.pdf)
...
* 含有 $n$ 个结点的二叉树最多有 $\dfrac{\mathrm{C}_{2n}^{n}}{n+1}=\dfrac{(2n)!}{n!(n+1)!}$ ，即：[卡特兰数](第3章%20栈和队列.md#栈的常见题型)
...
![二叉树的存储结构.pdf](03.5.2.2二叉树的存储结构.pdf)
...
![二叉树的先中后序遍历.pdf](01.5.3.1_1二叉树的先中后序遍历.pdf)

* 二叉树的遍历是指按照某种顺序访问树的每一个结点，使得每个结点均被访问到，而且仅访问一次。这里的访问是指对结点进行各种处理的统称，包括对结点数据进行运算、修改和打印信息等
* 对二叉树进行遍历是为了对二叉树进行某些操作或者获取树中的某些信息。
* **分支结点逐层展开法**：如果是一个结点是分支结点而不是叶子结点的话，就[嵌套递归](第3章%20栈和队列.md#3.3.3%20栈在递归中的应用)地按照特定的遍历序列把它展开到下一级
* 对算术表达式的“分析树”进行遍历，可以得到其表达式：先序遍历对应[前缀表达式](第3章%20栈和队列.md#前缀表达式%20（波兰表达式）)，中序遍历对应[中缀表达式](第3章%20栈和队列.md#中缀表达式（需要界限符）)，后序遍历对应[后缀表达式](第3章%20栈和队列.md#后缀表达式（逆波兰表达式）)
...
![二叉树的层次遍历.pdf](02.5.3.1_2二叉树的层次遍历.pdf)
...
![由遍历序列构造二叉树.pdf](03.5.3.1_3由遍历序列构造二叉树.pdf)
...
![线索二叉树的概念.pdf](04.5.3.2_1线索二叉树的概念.pdf)
...
![二叉树的线索化.pdf](05.5.3.2_2二叉树的线索化.pdf)
...
![在线索二叉树中找前驱后继.pdf](06.5.3.2_3在线索二叉树中找前驱后继.pdf)
...
![树、森林与二叉树的转换.pdf](02.5.4.2树、森林与二叉树的转换.pdf)
...
![树和森林的遍历.pdf](03.5.4.3树和森林的遍历.pdf)
...
![并查集的进一步优化.pdf](03.5.5.2_2并查集的进一步优化.pdf)
...
* [Kruskal算法](第6章%20图.md#Kruskal算法)：判断是否加入一条边之前，先查找这条边关联的两个顶点是否属于同一个集合(即判断加入这条边之后是否形成回路)，若形成回路， 则继续判断下一条边；若不形成回路，则将该边和边对应的顶点加入最小生成树T，并继续判断下一条边，直到所有顶点都已加入最小生成树 #考前记一记 
